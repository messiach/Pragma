...
select count(*) from epmbuildattribute;    -- count = 0 (NM)
select count(*) from epmderivedrephistory; -- count = 0 (NM)
select count(*) from epmderivedreprule;    -- count = 0 (NM)
...
select count(*) from epmcheckpoint;        -- count = 1,654   (TBM)
select count(*) from epmcheckpointmaster;  -- count = 809,860 (TBM)
...
select count(*) from epmcadnamespace;      -- count = 208,115 (TBM)

-- workspaces
select count(*) from epmworkspace;         -- count = 1,654   --> PDMLink user workspaces  (TBM)
...
select count(*) from epmdocconfigspec;     -- count = 716,438 (TBM)

-- as stored config
-- ** represents the configuration itself and the members of those configs
select count(*) from epmasstoredconfig;    -- count = 86,398      (TBM)
select count(*) from epmasstoredmember;    -- count = 15,394,851  (TBM)

-- authoring apps
select count(*) from epmauthoringappversion;   -- count = 97  (TBM)
...
select count(*) from epmsepfamilytable;       -- count = 13,218     (TBM)
select count(*) from epmfamilytablecell;      -- count = 2,494,600  (TBM)
select count(*) from epmfamilytablecelldep;   -- count = 27,465     (TBM)
select count(*) from epmfamilytablemember;    -- count = 12,465     (TBM)  
...
select count(*) from epmfamilytablefeature;   -- count = 39,629   --> CAD Authoring App feature defined on family table generic cad model - relates a feature definition directly to a family table (not the master) and provides the title for the feature              (TBM)
select count(*) from epmfeaturedefinition;    -- count = 5,847    --> CAD Authoring App feature defined on family table generic cad model (this is the type of feature) - relates directly to the master family table for the family table tree and has an internal id   (TBM)
select count(*) from epmfeaturevalue;         -- count = 390,187  --> CAD Authoring App feature as turned on or off - value will only ever be true/false (1/0 in 'numbervalue' column) - relates directly to the epmdocument only                                        (TBM)
...
select count(*) from epmfamilytableattribute; -- count = 6,609  (TBM)
...
select count(*) from epmfamilytableparameter; -- count = 40,297     (TBM)
...
select count(*) from epmcontainedin;       -- count = 248,395  (TBM)
